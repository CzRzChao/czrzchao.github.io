...
# 前言
本文探究的数据结构并不是 *redis* 对外暴露的5中数据结构，而是*redis*内部使用的基础数据结构，这些基础的数据结构 *redis* 不仅和 *redisObj* 一起构成了对外暴露的5中数据结构，还被运用于 *redis* 内部的各种存储和逻辑交互，支撑起了 *redis* 的运行。  
*redis*的基础数据结构主要有：  
...
### SDS的定义
#### 3.2和3.0的差别
**sds**在 *redis* 3.2版本做了较大的变动，这里就简单顺便介绍一下之前的**SDS**。在3.2以前**SDS**只有一种数据结构，为什么说只呢，因为到了3.2以后**SDS**根据存储的内容会选择不同的数据结构，以到达节省内存的效果！
...
如果不对齐cpu的的内存访问速度回大大下降，至于内存对齐的原理这里就不做赘述了，有兴趣可以自行google或百度。  
...
```
可以看到跟`sdshdr`同量级的`sdshdr16`多节省了3个字节的内存，效果显著。但是内存对齐怎么办呢，不能为了一点内存大大拖慢cpu的寻址效率啊？*redis* 自己在malloc等c语言传统的内存分配函数上封装了一层**zmalloc**，其中在内存分配时有这么一段代码：

```c
if (_n&(sizeof(long)-1)) _n += sizeof(long)-(_n&(sizeof(long)-1)); \    // 确保内存对齐！
```
这段代码写的比较抽象，简单描述就是先判断当前要分配的`_n`个内存是否是`long`类型的整数倍，如果不是就在`_n`的基础上加上内存大小差值，从而达到了内存对齐的保证。

#### SDS数据结构解读
扯了一大堆有点偏题了，回到正题，以3.2的**SDS**定义来解读一下这个数据结构。以`sdshdr8`为例：

![sdshdr8](/images/sdshdr8.png)  

* `len`记录当前字符串的长度（不包括`\0`），使得获取字符串长度的时间复杂度由*O(N)*变为了*O(1)*
* `alloc`记录了当前分配的内存大小
* `flags`记录了当前字符串的属性、用来标识到底是`sdshdr8`还是`sdshdr16`等
* `buf`保存了字符串真正的值以及末尾的一个`\0`

除此之外，整个**SDS**的内存是连续的，统一开辟的。为何要统一开辟呢？因为在大多数操作中，`buf`内的字符串实体才是操作对象。如果统一开辟内存就能通过`buf`寻址，拿到整个`struct`的指针，而且通过`buf`的头指针减一直接就能查找到`flags`的值，骚操作代码如下！
...
```
至于`buf`末尾的`\0`是为了复用C语言自带的部分字符串操作函数。
...
```
该方法是用来确保**SDS**字符串在拼接时有足够的空间，如果空间不够就会重新分配内存，内存的分配策略主要有两种：
